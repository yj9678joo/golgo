          ...
          <>
            ...
            <section className="rounded-[20px] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_4px_16px_rgba(0,0,0,0.04)]">
                ...
                {PERIODS.map((item) => (
                  <button
                    ...
                    className={`h-8 rounded-[9px] text-[11px] font-semibold transition ${
                      period === item ? 'bg-white text-[#191F28] shadow-sm' : 'text-[#8B95A1]'
                    }`}
                    type="button"
              ...
              <div className="mt-3 grid grid-cols-3 gap-3 border-t border-[#F2F4F6] pt-3">
                <AssetStat label="평가손익" value={`${formatCompactKrw(portfolio.totalProfitKrw)}원`} />
                <AssetStat label="수익률" value={formatSignedPercent(portfolio.profitRate)} />
                <AssetStat label="보유 종목" value={`${portfolio.holdings.length}개`} />
              </div>
            </section>
...
function AssetStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      <p className="truncate text-[11px] font-semibold text-[#8B95A1]">{label}</p>
      <p className="mt-1 truncate font-mono text-[13px] font-bold text-[#191F28]">{value}</p>
    </div>
  )
}
...
function QuickActionTile({
  ...
  return (
    ...
    >
      ...
      <span className="mt-3 block text-[14px] font-semibold text-[#191F28]">{label}</span>
      <span className="mt-1 block truncate text-[12px] font-medium text-[#8B95A1]">{sub}</span>
    </button>
  )
}
